...
router.get('/getUsers', function(req, res, next) {
  req.pool.getConnection(function(error, connection) {
    ...
    let query = "SELECT first_name, last_name, user_id, username, email, isAdmin FROM User";
    connection.query(query, [], function(error, rows, fields) {
      connection.release();
      if (error) {
        console.log(error);
...
router.post('/myEventsFinalTime', function(req, res, next) {
  console.log("in myeventsfinaltime");
  console.log(req.body);
  req.pool.getConnection(function(error, connection) {
    if (error) {
      console.log(error);
    ...
    let query = "UPDATE Event SET event_starttime = ?, event_endtime = ? WHERE event_id = ?;";
    connection.query(query, [req.body.start, req.body.end, req.body.id], function(error, rows, fields) {
      connection.release();
      if (error) {
        console.log(error);
        res.sendStatus(500);
